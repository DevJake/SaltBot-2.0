...
public class R6GameStatsCommand extends Command implements ICommand {
    ...
    public void execute(CommandParser.ParsedCommandContainer cmd, GuildMessageReceivedEvent e) {
        ...
        R6Player r = h.getPlayerByName("Wallek224", Platform.UPLAY); //TODO add count of R6 command calls to stats. Allow stats command to show stats on more individual things, such as usages of each command
//        e.getChannel().sendMessage(r.toString()).queue();
        e.getChannel().sendMessage(
                new EmbedBuilder()//TODO have R6Handler throw exception so that we can tell the user if the specified individual exists or not
                        ...
                        .addField("Headshots", String.valueOf(r.getStats().getOverall().getHeadshots()), true)
                        .addField("Bullets Fired", String.valueOf(r.getStats().getOverall().getBulletsFired()), true)
                        .addField("Bullets Hit", String.valueOf(r.getStats().getOverall().getBulletsHit()), true)
                        .addField("Suicides", String.valueOf(r.getStats().getOverall().getSuicides()), true)
                        .addField("Steps Moved", String.valueOf(r.getStats().getOverall().getStepsMoved()), true)
                        .build()
        ).queue();
    }
