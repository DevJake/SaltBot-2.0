        ...
        if (GameManager.hasGameOfType(e.getAuthor(), CaDGameHandler.class)) {
            ...
            toRemove.forEach(user -> {
                if (players.containsKey(user))
                    game.removePlayer(players.get(user));
                removed.add(user);
            });

            RestAction r = removed.size() > 0 ? e.getChannel().sendMessage("Removed the following users from the game: " + removed.toString()) : e.getChannel().sendMessage("No players matching your query were found. No users were removed from the game");
            r.queue();

        } else {
            e.getChannel().sendMessage("You don't currently have a Cards Against Humanity game session in progress!").queue();
        }
