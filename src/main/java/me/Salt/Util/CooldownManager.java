...
public class CooldownManager {
    ...
    public static boolean isNotInCooldown(String cmd, ICommand command, User user, long time) {
        ...
        if (cooldowns.containsKey(command)) {
            if (cooldowns.get(command).containsKey(user)) {
                if (Main.salt.getCommands()
                             .get(cmd)
                             .getCmdContainer()
                             .getCommandDescription()
                             .hasCooldown()) {//Does the command actually -have- a cooldown?
                    if (System.currentTimeMillis() - cooldowns.get(command).get(user) >= Main.salt.getCommands()
                                                                                                  .get(cmd)
                                                                                                  .getCmdContainer()
                                                                                                  .getCommandDescription()
                                                                                                  .getCooldown()
                ...
                return Main.salt.getCommands()
                                .get(cmd)
                                .getCmdContainer()
                                .getCommandDescription()
                                .hasCooldown(); //isNotInCooldown(cmd, command, user, time);
    ...
    public static long getRemainingTime(String cmd, ICommand command,
                                        User user) { //Return how long the individual has left
        //TODO convert remaining time to seconds.
        long called = cooldowns.get(command).get(user);
        long cooldown = Main.salt.getCommands()
                                 .get(cmd)
                                 .getCmdContainer()
                                 .getCommandDescription()
                                 .getCooldown()
