...
public class ProfileInfoCommand extends Command implements ICommand {
    ...
    @Override
    public boolean preExecution(CommandParser.ParsedCommandContainer cmd, GuildMessageReceivedEvent event) throws LackingPermissionException, MissingDataException {

        if (!Main.salt.getJUserById(event.getAuthor().getId()).hasPermission(Perm.COMMAND_PROFILE_USE))
            throw new LackingPermissionException("You lack the " + Perm.COMMAND_PROFILE_USE.name() + " permission!"); //TODO replace with getPermission call

        if (cmd.getArgsLower().size() >= 1) {
            if (!Main.salt.getJUserById(event.getAuthor().getId()).hasPermission(Perm.COMMAND_PROFILE_SET_USER))
                throw new LackingPermissionException("You lack the " + Perm.COMMAND_PROFILE_SET_USER.name() + " permission!"); //TODO replace with getPermission call

            return event.getGuild().getMembers().stream().filter(member -> member.getEffectiveName().toLowerCase().contains(cmd.getArgsLower().get(0))).collect(Collectors.toList()) != null && event.getGuild().getMembers().stream().filter(member -> member.getEffectiveName().toLowerCase().contains(cmd.getArgsLower().get(0))).collect(Collectors.toList()).size() == 1;
        } else return true;
    }
    ...
    public void execute(CommandParser.ParsedCommandContainer cmd, GuildMessageReceivedEvent e) {
        ...
        if (cmd.getArgsLower().size() > 0)
            m = e.getGuild().getMembers().stream().filter(member -> member.getEffectiveName().toLowerCase().contains(cmd.getArgsLower().get(0))).collect(Collectors.toList()).get(0);
        else m = e.getMember();
...
//TODO
        e.getChannel().sendMessage(
                new EmbedBuilder()
                        .setTitle("Profile details for " + m.getEffectiveName() + "#" + m.getUser().getDiscriminator(), null)
                        .addField("Name", m.getEffectiveName(), true)
                        .addField("Nickname", m.getNickname() != null ? m.getNickname() : "None set", true)
                        .addField("Current Game", m.getGame() != null ? m.getGame().toString() : "None set", true)
